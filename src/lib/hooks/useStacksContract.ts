...
const extractDonationAmount = async (campaignId: number, donor: string | undefined): Promise<number> => {
  if (!donor || !campaignId || isNaN(campaignId) || campaignId < 1) return 0;
  try {
    const raw = await getDonation(campaignId, donor);
...
export function useDonation(campaignId: number, donor: string | undefined) {
  ...
  const [isLoading, setIsLoading] = useState(false);
  useEffect(() => {
    extractDonationAmount(campaignId, donor).then((amount) => {
      setDonation(amount);
    }).catch(() => {})
      .finally(() => {
        setIsLoading(false);
