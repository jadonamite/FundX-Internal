...
const calculateDonationAmount = (raw: number): number => {
...
export function useAllCampaigns() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setError(null);
    fetchAllCampaigns().then(({ campaigns, count }) => {
      if (cancelled) return;
...
export function useCampaign(id: number) {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setError(null);
    Promise.all([getCampaignRaw(id), getBlockHeight(), getRegistryMeta(id)]).then(([raw, blockHeight, meta]) => {
      if (cancelled) return;
...
export function useDonation(campaignId: number, donor: string | undefined) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setIsLoading(true);
    getDonation(campaignId, donor).then((amount) => {
      if (cancelled) return;
