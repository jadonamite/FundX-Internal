...
// Helper function to check the status of a transaction
async function getTxStatus(txid: string): Promise<"success" | "failed" | null> {
  try {
    ...
    // network hiccup — return null to indicate failure to fetch
    return null;
  }
  return null;
}
