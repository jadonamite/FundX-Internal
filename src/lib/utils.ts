...
const getTxStatus = async (txid: string): Promise<{ status: "success" | "failed" | null; data: any }> => {
  try {
    const res = await fetch(`${HIRO_API}/extended/v1/tx/${txid}`);
    if (res.ok) {
      const data = await res.json();
      if (data.tx_status === "success") return { status: "success", data };
      if (data.tx_status === "abort_by_response" || data.tx_status === "abort_by_post_condition") return { status: "failed", data };
      return { status: null, data };
    }
  } catch {
    // network hiccup — return null status
