...
function getAssetNameFromContractName(contractName: string, assetNames: Record<string, string>): string {
  return assetNames[contractName] ?? contractName
}

export function getTokenAssetName(contractName: string): string {
  return getAssetNameFromContractName(contractName, TOKEN_ASSET_NAMES)
}
