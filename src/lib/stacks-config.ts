...
const extractContractAddress = (fqn: string): string => {
  const dot = fqn.lastIndexOf('.');
  return fqn.slice(0, dot);
};

const extractContractName = (fqn: string): string => {
  const dot = fqn.lastIndexOf('.');
  return fqn.slice(dot + 1);
};
...
const getTokenAssetName = (contractName: string): string => TOKEN_ASSET_NAMES[contractName] ?? contractName;

export function parseTokenFqn(fqn: string): [string, string] {
  return [extractContractAddress(fqn), extractContractName(fqn)];
}

export { getTokenAssetName };
