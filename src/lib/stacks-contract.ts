...
import { CONTRACT_ADDRESS, CONTRACT_NAME, REGISTRY_CONTRACT_NAME, STACKS_NETWORK, USDCX_DECIMALS, BLOCKS_PER_DAY, HIRO_API, } from "./stacks-config";

export interface RawCampaign {
  creator: string;
  assetStx: boolean; // true = native STX campaign, false = SIP-010 (USDCx) token
...
function calculateCampaignStatus(
  ...
  else if (isFlexible || raised >= goal) return "successful";
  else return "failed";
}
...
export function mapCampaign(
  ...
  const daysLeft = currentBlock >= deadlineBlock ? 0 : Math.ceil(blocksLeft / BLOCKS_PER_DAY);
  const status = calculateCampaignStatus(deadlineBlock, currentBlock, isFlexible, raised, goal);
  return {
    id: String(id),
...
export async function fetchAllCampaigns(): Promise<{ campaigns: OnChainCampaign[]; count: number; blockHeight: number }> {
