...
export interface RawCampaign {
  creator: string;
  assetStx: boolean; // true = native STX campaign, false = SIP-010 (USDCx) token: string | null; // FT contract FQN when assetStx is false; null for STX
  goal: bigint;
  deadline: bigint;
  totalRaised: bigint;
  withdrawn: boolean;
...
export function mapCampaign(
  ...
  const status = calculateCampaignStatus(isPast, isFlexible, raised, goal);
  return {
    id: String(id),
