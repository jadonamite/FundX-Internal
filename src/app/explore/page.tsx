    ...
    <main className="min-h-screen bg-slate-50 selection:bg-orange-100 font-sans relative">
      ...
      <div className="pt-32 pb-20 container mx-auto max-w-7xl px-4 relative z-10">
        ...
        <div className="mb-12 text-center max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm text-xs font-bold_ text-slate-500 mb-6">
             <Sparkles className="w-3 h-3 text-orange-500" />
             <span>{CAMPAIGNS.length} Active Campaigns</span>
          </div>
          <h1 className="text-5xl md:text-6xl font-bold_ text-slate-900 tracking-tight mb-6">
             ...
             <div className="flex gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 no-scrollbar items-center">
                {CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={`
                      px-5 py-2.5 rounded-xl text-sm font-bold_ whitespace-nowrap transition-all duration-300
             ...
             <div className="flex gap-2">
                {STATUSES.map((status) => (
                  <button
                    ...
                    className={`
                      px-4 py-1.5 rounded-lg text-xs font-bold_ uppercase tracking-wider transition-all duration-300 border whitespace-nowrap
                      ${statusFilter === status 
        ...
        {filteredCampaigns.length > 0 ? (
          ...
          <div className="text-center py-20 animate-in fade-in zoom-in duration-500">
             ...
             <h3 className="text-2xl font-bold_ text-slate-900 mb-2">No campaigns found</h3>
        ...
        {filteredCampaigns.length > visibleCount && (
          <div className="mt-16 text-center animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-500">
            <Button 
              ...
              variant="outline" 
              className="h-14 px-10 rounded-full border-2 border-slate-200 text-slate-600 font-bold_ hover:bg-white hover:border-slate-900 hover:text-slate-900 hover:scale-105 transition-all shadow-sm"
            >
              Load More Campaigns
            </Button>
          </div>
