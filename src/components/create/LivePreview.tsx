...
const getAssetClass = (isSTX: boolean) => {
  return isSTX ? 'bg-orange-100 text-orange-600' : 'bg-blue-50 text-blue-600'
}

const MetadataPreview = ({ formData }: { formData: CreateCampaignData }) => {
  ...
  return (
    <div className="bg-white/50 backdrop-blur border border-slate-200 p-4 rounded-xl text-sm space-y-2 max-w-[320px] mx-auto mt-4">
      ...
      <div className="flex items-center gap-2 text-slate-500">
        ...
        <span className={`font-bold px-2 py-0.5 rounded-md text-xs ${getAssetClass(isSTX)}`}
        >
