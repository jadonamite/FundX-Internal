    ...
    const checkConnection = async () => {
      try {
        ...
        if (isConnected()) {
          ...
          if (payload?.addresses?.stx?.[0]?.address) {
            setWalletData({
              stxAddress: payload.addresses.stx[0].address,
              btcAddress: payload.addresses.btc?.[0]?.address,
            })
            setIsSignedIn(true)
          }
        }
  ...
  const authenticate = async () => {
    try {
      ...
      const btcEntry = response.addresses.find(
        (addr) => addr.address?.startsWith('bc1') || addr.address?.startsWith('tb1')
      )
