...
export function ConnectWallet() {
  ...
  const copyAddress = () => {
    if (walletData?.stxAddress) {
      ...
      toast.info("Address Copied", { description: "Copied to clipboard", duration: 2000 })
    }
  }

  const handleDisconnect = () => {
    signOut()
    toast.error("Disconnected", { description: "Session ended securely." })
  }

  const renderWalletConnectionState = () => {
    if (!mounted) {
      return (
    ...
    if (isSignedIn && walletData?.stxAddress) {
      ...
      return (
        <div className="relative inline-flex">
          {/* Wrapper for positioning the ping */}
          {justConnected && (
            <span className="absolute -inset-1 rounded-full bg-green-500 opacity-75 animate-ping duration-1000" />
          )}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button className={` relative z-10 rounded-full px-6 font-bold tracking-tight transition-all duration-500 ${justConnected ? "bg-green-500 border-green-400 text-white shadow-lg" // Success State : "bg-gradient-tush text-white shadow-glow hover:opacity-90 hover:scale-105" // Normal State } `}>
  ...
  return renderWalletConnectionState()
}
