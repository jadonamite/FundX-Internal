...
const renderDashboardLink = (isSignedIn: boolean) => {
  if (isSignedIn) {
    return (
    ...
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
      <nav className="flex w-full max-w-6xl items-center justify-between rounded-full bg-white/80 px-6 py-3 shadow-soft-md backdrop-blur-md border border-white/20">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-500">
          ...
          <Link href="https://bridge.stacks.co/usdc/eth/stx" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">
            Bridge
          </Link>
