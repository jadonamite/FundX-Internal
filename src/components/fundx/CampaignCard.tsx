...
const getExcerpt = (tagline: string | undefined, description: string) => tagline || description;
const getFormattedCurrency = (value: number, currency: 'USDCx' | 'STX') =>
  currency === 'USDCx' ? `$${value.toLocaleString()}` : `${value.toLocaleString()} STX`;
const getPercentage = (raised: number, goal: number) => Math.min((raised / goal) * 100, 100);
...
export function CampaignCard({
  ...
  const excerpt = getExcerpt(tagline, description);
  const percentage = getPercentage(raised, goal);
  const formattedRaised = getFormattedCurrency(raised, currency);
  const formattedGoal = getFormattedCurrency(goal, currency);
