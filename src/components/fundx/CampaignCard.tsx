    ...
    <Link href={`/campaigns/${id}`} className="block h-full group">
      <Card className="relative h-full overflow-hidden rounded-3xl border-none bg-white p-6 shadow-soft-md transition-all duration-300 hover:-translate-y-1 hover:shadow-soft-xl">
        ...
        <div className="space-y-4">
          <div>
              <h3 className="text-xl font-bold_ text-slate-900 group-hover:text-primary transition-colors">{title}</h3>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm font-medium">
              ...
              <span className="text-primary font-bold_">{formattedRaised}</span>
              <span className="text-slate-400">of {formattedGoal}</span>
            </div>
            <Progress value={percentage} className="h-3 rounded-full bg-slate-100" />
          </div>

          <Button className="w-full rounded-xl bg-slate-900 text-white shadow-lg transition-all duration-500 hover:bg-gradient-tush hover:shadow-glow hover:scale-[1.02] py-6 text-base font-bold_">
            Donate Now
          </Button>
        </div>
      </Card>
