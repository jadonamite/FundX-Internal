...
const getToggleSwitchStyles = (isStacksMode: boolean) => ({
  container: {
  ...
  return (
    <span className="inline-flex align-middle ml-2">
      <button onClick={onToggle} aria-label="Toggle between Bitcoin and Stacks" className="relative inline-flex items-center cursor-pointer focus:outline-none" style={{ WebkitTapHighlightColor: 'transparent' }}>
        <div style={getToggleSwitchStyles(isStacksMode).container} className="w-24 h-12 rounded-full p-1">
          <div style={getToggleSwitchStyles(isStacksMode).toggle} className="w-10 h-10 bg-white rounded-full" />
        </div>
      </button>
    </span>
  )
