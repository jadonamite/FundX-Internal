...
const getButtonStyles = (isStacksMode: boolean) => ({
  background: isStacksMode ? '#0f172a' : 'linear-gradient(to right, #FF6B4A, #FF3D71)',
  transition: 'background 700ms ease'
});

const getToggleStyles = (isStacksMode: boolean) => ({
  ...
  return (
    <span className="inline-flex align-middle ml-2">
      <button onClick={onToggle} aria-label="Toggle between Bitcoin and Stacks" className="relative inline-flex items-center cursor-pointer focus:outline-none" style={{ WebkitTapHighlightColor: 'transparent' }}>
        <div style={getButtonStyles(isStacksMode)} className="w-24 h-12 rounded-full p-1">
          <div style={getToggleStyles(isStacksMode)} className="w-10 h-10 bg-white rounded-full" />
        </div>
      </button>
    </span>
  )
