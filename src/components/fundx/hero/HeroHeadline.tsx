...
export function HeroHeadline({
  displayStacks, glitching, glitchOffset, glitchOpacity, glitchSkew, isStacksMode_, scrambledText, onToggle,
}: {
  displayStacks: boolean; glitching: boolean; glitchOffset: { x: number; y: number }; glitchOpacity: number; glitchSkew: number; isStacksMode_: boolean; scrambledText: string; onToggle: () => void
}) {
  return (
    <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold tracking-tighter text-slate-900 leading-[1.1] mb-8">
      ...
      <span className="inline-flex items-center flex-wrap justify-center gap-x-4">
        on the
        <ChainToggleIcon displayStacks={displayStacks} glitching={glitching} glitchOffset={glitchOffset} glitchOpacity={glitchOpacity} glitchSkew={glitchSkew} isStacksMode_={isStacksMode_} />
