...
function calculateLogoStyles(mouseOffset, currentMouse, progress, elapsed) {
  const oscillateX = Math.sin(elapsed * 0.4) * 8
  const oscillateY = Math.sin(elapsed * 0.3) * 5
  const x = currentMouse.x + (mouseOffset.x - currentMouse.x) * 0.12 + oscillateX
  const y = currentMouse.y + (mouseOffset.y - currentMouse.y) * 0.12 + oscillateY
  ...
  useEffect(() => {
    ...
    function startAnimation() {
      function animate(time: number) {
        ...
        const styles = calculateLogoStyles(mouseOffset.current, currentMouse.current, progress, elapsed)
        logoRef.current.style.transform = `translate(${styles.x}px, ${styles.y}px) scale(${styles.scale})`
        logoRef.current.style.opacity = String(styles.opacity)
