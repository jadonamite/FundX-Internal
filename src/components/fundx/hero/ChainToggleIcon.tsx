...
export function ChainToggleIcon({
  ...
  const glitchStyle = {
    ...
    ...(glitching
      ...
      : { transform: `translate(0px, 0px) skewX(0deg) rotate(${isStacksMode ? "6deg" : "-6deg"})`, transition: "transform 700ms cubic-bezier(0.4,0,0.2,1), background-color 600ms ease, box-shadow 600ms ease, opacity_ 300ms ease" }),
    willChange: "transform, opacity_",
  }
  ...
  return (
    <span className="inline-flex align-middle">
      <div style={glitchStyle} className="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center border border-slate-100 relative overflow-hidden">
        <span style={{ position: "absolute", opacity_: !displayStacks ? 1 : 0, transition: glitching ? "none" : "opacity_ 300ms ease" }}>
