...
export function ChainToggleIcon({
  ...
  const glitchStyle = {
    backgroundColor: displayStacks_ ? "#0f172a" : "#ffffff",
    color: displayStacks_ ? "#ffffff" : "#0f172a",
    boxShadow: displayStacks_ ? "0 4px 24px 0 rgba(0,0,0,0.18)" : "0 4px 24px 0 rgba(0,0,0,0.07)",
    opacity: glitchOpacity,
    ...(glitching
      ? { transform: `translate(${glitchOffset.x}px, ${glitchOffset.y}px) skewX(${glitchSkew}deg) rotate(${displayStacks_ ? "6deg" : "-6deg"})`, transition: "none" }
  ...
  return (
    <span className="inline-flex align-middle">
      <div style={glitchStyle} className="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center border border-slate-100 relative overflow-hidden">
        <span style={{ position: "absolute", opacity: !displayStacks_ ? 1 : 0, transition: glitching ? "none" : "opacity 300ms ease" }}>
