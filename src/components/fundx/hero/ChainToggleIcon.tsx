...
const getGlitchStyle = ({ displayStacks, glitching, glitchOffset, glitchOpacity, glitchSkew, isStacksMode }: { displayStacks: boolean, glitching: boolean, glitchOffset: { x: number, y: number }, glitchOpacity: number, glitchSkew: number, isStacksMode: boolean }) => ({
...
const getIconStyle = (displayStacks: boolean, glitching: boolean) => ({
  position: "absolute",
  opacity: displayStacks ? 1 : 0,
  transition: glitching ? "none" : "opacity 300ms ease"
})

const getBitcoinIconStyle = (displayStacks: boolean, glitching: boolean) => ({
  position: "absolute",
  opacity: !displayStacks ? 1 : 0,
  transition: glitching ? "none" : "opacity 300ms ease"
})
...
export function ChainToggleIcon({
  ...
  const glitchStyle = getGlitchStyle({ displayStacks, glitching, glitchOffset, glitchOpacity, glitchSkew, isStacksMode })
  const iconStyle = getIconStyle(displayStacks, glitching)
  const bitcoinIconStyle = getBitcoinIconStyle(displayStacks, glitching)
