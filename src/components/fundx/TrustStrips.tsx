export function TrustStrip() {
  const trustFeatures = [
    { iconColor: 'emerald-500', label: 'USDCx-backed' },
    { iconColor: 'indigo-500', label: 'On-chain Escrow' },
    { iconColor: 'purple-500', label: 'SIP-010 Compatible' },
    { iconColor: 'pink-500', label: 'Non-Custodial' },
  ];

  return (
    <div className="mt-12 relative">
      <div className="absolute inset-0 bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-pink-500/10 blur-2xl opacity-70 rounded-3xl" />

      <div className="relative backdrop-blur-xl border border-white/40 bg-white/70 rounded-3xl px-8 py-5 shadow-lg">
        <div className="flex flex-wrap justify-center gap-8 text-sm font-semibold text-slate-700 tracking-wide">
          {trustFeatures.map((feature) => (
            <span key={feature.label} className="flex items-center gap-2">
              <span className={`h-2 w-2 rounded-full bg-${feature.iconColor} animate-pulse`} />
              {feature.label}
            </span>
          ))}
        </div>
      </div>
    </div>
  );
}
