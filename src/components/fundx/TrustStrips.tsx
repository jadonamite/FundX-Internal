export function TrustStrip() {
  const trustStripItems = [
    { color: 'emerald-500', label: 'USDCx-backed' },
    { color: 'indigo-500', label: 'On-chain Escrow' },
    { color: 'purple-500', label: 'SIP-010 Compatible' },
    { color: 'pink-500', label: 'Non-Custodial' },
  ];

  const renderTrustStripItem = (item: { color: string; label: string }) => (
    <span className="flex items-center gap-2">
      <span className={`h-2 w-2 rounded-full bg-${item.color} animate-pulse`} />
      {item.label}
    </span>
  );
    ...
    <div className="mt-12 relative">
      ...
      <div className="relative backdrop-blur-xl border border-white/40 bg-white/70 rounded-3xl px-8 py-5 shadow-lg">
        <div className="flex flex-wrap justify-center gap-8 text-sm font-semibold text-slate-700 tracking-wide">
          {trustStripItems.map(renderTrustStripItem)}
        </div>
      </div>
    </div>
  );
