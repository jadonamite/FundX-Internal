export function TrustStrip() {
  const TrustIndicator = ({ color, label }: { color: string, label: string }) => (
    <span className="flex items-center gap-2">
      <span className={`h-2 w-2 rounded-full bg-${color}-500 animate-pulse`} />
      {label}
    </span>
  );
    ...
    <div className="mt-12 relative">
      ...
      <div className="relative backdrop-blur-xl border border-white/40 bg-white/70 rounded-3xl px-8 py-5 shadow-lg">
        <div className="flex flex-wrap justify-center gap-8 text-sm font-semibold text-slate-700 tracking-wide">
          <TrustIndicator color="emerald" label="USDCx-backed" />
          <TrustIndicator color="indigo" label="On-chain Escrow" />
          <TrustIndicator color="purple" label="SIP-010 Compatible" />
          <TrustIndicator color="pink" label="Non-Custodial" />
        </div>
      </div>
    </div>
  )
