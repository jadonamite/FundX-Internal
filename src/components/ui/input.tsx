...
const getInputClassName = (className: string, isValid: boolean) => {
  return cn(
...
function Input({ className, type, 'aria-invalid': isValid, ...props }: React.ComponentProps<"input">
) {
  return (
    <input
      type={type}
      data-slot="input"
      className={getInputClassName(className, !isValid)}
      {...props}
    />
  );
}
