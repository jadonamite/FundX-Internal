...
| FundX Escrow | `SP6X0MXEEGZX14ZTK7XQXJ76W35ZJDP9NZBT6F39.fundx-escrow-v2` |
| USDCx Token | `SP6X0MXEEGZX14ZTK7XQXJ76W35ZJDP9NZBT6F39.usdcx-v2` (6 dec) |
...
1. **Single token** — only USDCx, no multi-token support like Celo's cUSD/USDC. Currency is always `"USDCx"`.
...
7. **Refetch is timer-based after writes** — Stacks doesn't have wagmi's `waitForTransactionReceipt`. After a write, we use `setTimeout(refetch, 8000)` to give the mempool a chance to confirm. Could improve by polling tx status via `/extended/v1/tx/{txid}`.
...
2. **No tx confirmation polling** — 8s sleep + refetch is heuristic. A real `useTransactionStatus(txid)` hook would be cleaner.
...
| Tx confirmation | `waitForTransactionReceipt(config, { hash })` | `setTimeout(refetch, 8000)` (workaround) |
